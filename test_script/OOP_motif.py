...
class Fasta():
    def __init__(self,Fasta_file,out):
        self.Fasta_file=Fasta_file
        self.out=out
    
    def single_fasta(self):
        print("converting to single line fasta")
        single_dict={}
        ...
        out_fasta=self.out+"_single_fasta.fa"

        with open(out_fasta,"w") as sfa:
    ...
    def parse_fasta(self):
        ...
        out_fasta=Fasta.single_fasta(Fasta_file)    

        return out_fasta
        
fasta_obj=Fasta(Fasta_file,out)
fasta_obj.parse_fasta()
#
...
surface=cairo.SVGSurface("C:/Bi624/motif-mark/tony.svg", 1000, 800)
ctx = cairo.Context(surface)
x=20

f="C:/Bi624/motif-mark/single_fasta.txt"
file_open=open(f)
for line in file_open:
    ...
    if ">" in line.strip():
        ...
        ctx.rectangle(100, x, 80, x)
        ctx.stroke()
        x+=10
        
        
with open(f) as fa:
    if ">" in fa.readline().strip()
for line in file_open:
#    print(line.strip())
    if ">" in line.strip():
        print(line)
